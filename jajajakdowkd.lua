...
function SetAutoload(configName)
	if writefile and AutoloadFile then
		...
		print("[Zero] Autoload config set to: " .. configName)
	end
end
...
function ClearAutoload()
	if delfile and AutoloadFile and isfile(AutoloadFile) then
		...
		print("[Zero] Autoload config cleared")
	end
end

function SaveConfigAs(configName)
	if not configName or configName == "" then
		warn("[Zero] Config name cannot be empty")
		return false
	end
	...
	if writefile and gameFolder then
		...
		print("[Zero] Config saved as: " .. configName)
		return true
	end
	return false
end
	...
	if isfile and isfile(filepath) then
		...
		if success and type(result) == "table" then
			if result._version == CURRENT_VERSION then
				...
				CurrentConfigName = configName
				LoadConfigElements()
				print("[Zero] Config loaded: " .. configName)
				return true
			else
				warn("[Zero] Config version mismatch")
				return false
			end
		else
			warn("[Zero] Failed to decode config")
			return false
		end
	else
		warn("[Zero] Config file not found: " .. configName)
		return false
	end
end
...
function DeleteConfig(configName)
	...
	if delfile and isfile(filepath) then
		delfile(filepath)
		print("[Zero] Config deleted: " .. configName)
...
function ResetElements()
	...
	print("[Zero] Elements reset to defaults")
end

function LoadExternalConfigFromJSON(jsonString)
	if not jsonString or jsonString == "" then
		warn("[Zero] External config JSON kosong")
		return false
	end
	...
	if not ok or type(data) ~= "table" then
		warn("[Zero] External config JSON invalid")
		return false
	end

	if data._version and CURRENT_VERSION and data._version ~= CURRENT_VERSION then
		warn("[Zero] External config version mismatch")
		return false
	end
	...
	print("[Zero] External config loaded from JSON input")
	return true
end
...
local function ShowLoading()
	...
	title.TextSize = 15
	title.TextXAlignment = Enum.TextXAlignment.Left
	title.Text = "Zero Impact ANNOUNCEMENT"
	...
	msg.Text = "Zero Impact Script are free script!"
...
function Chloex:Window(GuiConfig)
	if not _G.__Zero_LOADED then
		_G.__Zero_LOADED = true
		ShowLoading()
		task.wait()
	end

	GuiConfig = GuiConfig or {}
	GuiConfig.Title = GuiConfig.Title or "Zero Impact"
	...
	gameFolder = "ZeroImpact/" .. folderName
